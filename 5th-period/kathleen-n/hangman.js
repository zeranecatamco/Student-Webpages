//Initial References, each container or div created in html
 ...
 const generateWord = (optionValue) => {
   let optionsButtons = document.querySelectorAll(".options");
   //If optionValue matches the button innerText then highlight the button (only works for first category)
   optionsButtons.forEach((button) => {
     if (button.innerText.toLowerCase() === optionValue) { // allow lowercase letters, options for letter button are all upper case
